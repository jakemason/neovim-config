...
function M.run(build_path)
	...
	vim.notify("Executing build @ " .. build_path .. "...", vim.log.levels.INFO, { title = "Build" })
	...
	if ok and exit_code == 0 then
		vim.notify("Build completed.", vim.log.levels.INFO, { title = "Build" })
		return
	end
	...
	vim.notify(message, vim.log.levels.ERROR, { title = "Build Error" })
end
